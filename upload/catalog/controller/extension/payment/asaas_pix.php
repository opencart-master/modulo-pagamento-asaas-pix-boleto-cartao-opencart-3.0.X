...
class ControllerExtensionPaymentAsaasPix extends Controller {
	public function index() {
		...
		return $this->load->view('extension/payment/asaas_pix', $data);
	}
	...
	public function confirm() {
		...
		if (isset($this->session->data['payment_method']['code']) && $this->session->data['payment_method']['code'] == 'asaas_pix') {
			...
			$asaas = new AsaasApi($this->config->get('payment_asaas_pix_api_key'), $mode);

			$getcustomer = $asaas->getCustomer($order_info['email']);

			foreach ($custom as $key => $value) {
				if ($this->config->get('payment_asaas_pix_doc') == $key && !empty($value)) {
                   $doc = $value;
				} 
				
				if ($this->config->get('payment_asaas_pix_doc1') == $key && !empty($value)) {
				   $doc = $value;
				}
			}
